    ...
    def __getitem__(self, index):
        ...
        if(self.inference):
            return (query_rgb, query_th, query_d, query_mask.astype(np.int64), 
                    ...
                    np.array(sample_class-1).astype(np.int64), query_name_rgb)
        else:
            return (query_rgb, query_th, query_d, query_mask.astype(np.int64), 
                    support_rgb,support_th, support_d, support_mask.astype(np.int64), 
...
class Tri_Dataset(object):
    ...
    def __getitem__(self, index):
        ...
        if(self.mode == 'vis'):
            ...
            return (query_rgb, query_th, query_d, query_mask.astype(np.int64), 
                    support_rgb,support_th, support_d, support_mask.astype(np.int64), 
                    np.array(sample_class-1).astype(np.int64), query_name_rgb, ori)
...
class Tri_Dataset_test(object):
    ...
    def __getitem__(self, index):
        ...
        scale_transform_mask = P.Resize(self.input_size, interpolation=P.Inter.NEAREST)
        scale_transform_rgb = P.Resize(self.input_size, interpolation=P.Inter.BILINEAR)
        scale_transform_th = P.Resize(self.input_size, interpolation=P.Inter.BILINEAR)
        scale_transform_d = P.Resize(self.input_size, interpolation=P.Inter.BILINEAR)
        ...
        if(self.mode == 'vis'):
            ori_query_rgb = np.array(ori_query_rgb)
            ori_query_d = np.array(ori_query_d)
            ori_query_th = np.array(ori_query_th)
            
            ori_query_mask = np.array(ori_query_mask)
            if(len(ori_query_mask.shape) == 3):
                ori_query_mask = ori_query_mask[:,:,0]

            ori_support_rgb = np.array(ori_support_rgb)
            ori_support_d = np.array(ori_support_d)
            ori_support_th = np.array(ori_support_th)
            ori_support_mask = np.array(ori_support_mask)
            
            if(len(ori_support_mask.shape) == 3):
                ori_support_mask = ori_support_mask[:,:,0]
            return (query_rgb, query_th, query_d, query_mask.astype(np.int64), 
                    support_rgb, support_th, support_d, support_mask.astype(np.int64), 
                    np.array(sample_class-1).astype(np.int64),
                    ori_query_rgb, ori_query_th, ori_query_d, ori_query_mask, ori_support_rgb,ori_support_th, ori_support_d, ori_support_mask, query_name.split('.')[0])
        else:
            return (query_rgb, query_th, query_d, query_mask.astype(np.int64), 
                    support_rgb, support_th, support_d, support_mask.astype(np.int64), 
                    np.array(sample_class-1).astype(np.int64))
