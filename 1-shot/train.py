...
def main(model):
    ...
    for epoch in range(start_epoch,args.epoch):
        trn_loss, trn_miou, trn_fb_iou = train(epoch, model, train_loader, optimizer, sub_list, training=True, auxloss_weight = 0.3)
        with torch.no_grad():
...
if __name__ == '__main__':
    ...
    if(args.model == 'IFCNet'):
        Logger.initialize(args, training=True, modelname="IFCNet")
        model = IFCNet(args.backbone, False, dropout=args.drop)
    elif(args.model == 'SEMA'):
        Logger.initialize(args, training=True, modelname="SEMA")
        model = SEMA(args.backbone, False, dropout=args.drop)
