...
def visulize(ori_query_rgb,ori_query_d,ori_query_th,q_mask, ori_support_rgb,ori_support_d,ori_support_th,s_mask, ori_name, q_pred,i):
    ...
    query_gt = Image.fromarray(apply_mask(ori_query_rgb.copy(),q_mask,colors_dict['red']))
    query_gt.save(os.path.join(path1,ori_name+'_query_GT.png'))

    query_rgb = Image.fromarray(apply_mask(ori_query_rgb.copy(),q_pred,colors_dict['blue']))
    query_rgb.save(os.path.join(path1,ori_name+'_query_pred.png'))

    support_gt = Image.fromarray(apply_mask(ori_support_rgb.copy(),s_mask,colors_dict['red']))
...
if __name__ == '__main__':
    ...
    if(args.model == 'IFCNet'):
        ...
        model = IFCNet(args.backbone, False)
