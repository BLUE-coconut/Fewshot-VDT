...
from model.hsnet import SEMA,HypercorrSqueezeNetwork
...
def visulize(ori_query_rgb,ori_query_d,ori_query_th,q_mask, ori_support_rgb,ori_support_d,ori_support_th,s_mask, ori_name, q_pred,i):
    ...
    query_pred.save(os.path.join(path1,ori_name+'_query_pred.png'))
...
if __name__ == '__main__':
    ...
    if(args.model == '345-45-V'):
        Logger.initialize(args, training=True, modelname="HS_VDT_v2_10")  
        model = HS_VDT_v2_10(args.backbone, False)
    elif(args.model == 'sema'):
        Logger.initialize(args, training=True, modelname="SEMA")
        model = SEMA(args.backbone, False)
    elif(args.model == '345-345'):
        Logger.initialize(args, training=True, modelname="MCS345l_TC345l")
        model = MCS345l_TC345l(args.backbone, False)
    elif(args.model == '45-45'):
        Logger.initialize(args, training=True, modelname="MCS45l_TC45l")
        model = MCS45l_TC45l(args.backbone, False)
    elif(args.model == '345pro-345'):
        Logger.initialize(args, training=True, modelname="MCSpro345l_TC345l")
        model = MCSpro345l_TC345l(args.backbone, False)
    elif(args.model == '345pro-45'):
        Logger.initialize(args, training=True, modelname="MCSpro345l_TC45l")
        model = MCSpro345l_TC45l(args.backbone, False)    
    elif(args.model == 'wo-345'):
        Logger.initialize(args, training=True, modelname="MCSwo_TC345l")
        model = MCSwo_TC345l(args.backbone, False)
    elif(args.model == 'wo-45'):
        Logger.initialize(args, training=True, modelname="MCSwo_TC45l")
        model = MCSwo_TC45l(args.backbone, False)
    elif(args.model == '345-wo'):
        Logger.initialize(args, training=True, modelname="MCS345l_TCwo")
        model = MCS345l_TCwo(args.backbone, False)  
    elif(args.model == '345-45-D'):
        Logger.initialize(args, training=True, modelname="MCS345l_TC45l_D")
        model = MCS345l_TC45l_D(args.backbone, False) 
    elif(args.model == 'wo-45-D'):
        Logger.initialize(args, training=True, modelname="MCSwo_TC45l_D")
        model = MCSwo_TC45l_D(args.backbone, False)  
    elif(args.model == '345-45-T'):
        Logger.initialize(args, training=True, modelname="MCS345l_TC45l_T")
        model = MCS345l_TC45l_T(args.backbone, False)  
