...
class Attention(nn.Module):
  ...
  def forward(
    ...
    scores = torch.matmul(query, key.transpose(-2, -1))  # scores: bsz,H*W,H*W
    ...
    p_attn = F.softmax(scores, dim=-1)  # p_attn: bsz,H*W

    if dropout is not None:
      p_attn = dropout(p_attn)

    output = torch.matmul(p_attn, value)  # value: bsz,num_head,H*W,head_size
...
class MultiHeadedAttention(nn.Module):
  ...
  def forward(self, query, key, value, mask=None, one_direction=True, prefix=None):

    # 1) do all the linear projections in batch from d_model => num_heads x d_k
    bsz,ch,h,w = query.shape
    query = query.permute(0,2,3,1).view(bsz, -1, ch) # bsz, h*w, d_model
    key = key.permute(0,2,3,1).view(bsz, -1, ch)
    value = value.permute(0,2,3,1).view(bsz, -1, ch)
    ...
    if prefix is not None:
        prefix_key_, prefix_value_ = prefix[0].unsqueeze(0), prefix[1].unsqueeze(0)  # (1, T, num_heads, head_size)
        prefix_key_ = prefix_key_.repeat(bsz, 1, 1, 1).transpose(1, 2)  # (B, num_heads, T, head_size)
        prefix_value_ = prefix_value_.repeat(bsz, 1, 1, 1).transpose(1, 2)
        key_ = torch.cat([prefix_key_, key_], dim=2)
        value_ = torch.cat([prefix_value_, value_], dim=2)
        prefix_mask = torch.ones(
            (1, 1, 1, prefix_key_.shape[2]), dtype=mask.dtype).to(mask.device)
    ...
    return self.layer_norm(x + query).permute(0,2,1).view(bsz,ch,h,w)# bsz, d_model, h, w
