...
class MCS45l_TC45l(nn.Module):
    ...
    def __init__(self, backbone, use_original_imgsize, dropout=0.0, vis = False):
        ...
        # self.stack_ids = torch.tensor(self.lids).bincount().__reversed__().cumsum(dim=0)[:3]
        self.layer_ids = np.array([0]+self.nbottlenecks[1:]).cumsum()
        self.nbottlenecks = self.nbottlenecks[2:]
        self.backbone.eval() # backbone frozen

        # self.corr_mid = corr_mid*3 if len(corr_mid)==1 else corr_mid # correspond to l2,l3,l4

        self.multi_smi_fuse = nn.ModuleList([ConvModule(self.nbottlenecks[i]*3,self.nbottlenecks[i],1) 
                                             for i in range(len(self.nbottlenecks))]) # correspond to l3,l4
        ...
        self.Fusion = ConvModule(self.tri_out_ch+self.nbottlenecks[-1],self.tri_mid_ch,1)
        self.deblock1 = ConvModule(self.tri_out_ch+self.tri_mid_ch+self.nbottlenecks[-2],self.tri_out_ch,1)
        self.deblock2 = ConvModule(self.tri_out_ch,self.tri_out_ch,1)
    ...
    def forward(self, query_img, query_img_th, query_img_d,  support_img, support_img_th, support_img_d, support_mask):
        ...
        for idx,(qv_layer,sv_layer,qd_layer,sd_layer,qt_layer,st_layer) in enumerate(zip(query_feats,support_feats,query_feats_d,
            ...
            trisim = self.multi_smi_fuse[idx-1](torch.cat(vsim+dsim+tsim,dim=1))
            multi_sim.append(trisim)
            if self.vis:
                vsim_maps.append(vsim)
                dsim_maps.append(dsim)
        ...
        out1 = self.deblock2(self._upsample_like(out2,support_feats[-3][-1]))
        logit_masks.append(self.side1(out1))
        ...
        # decoder output 2 channel: indicate probabilities of foreground and background
        if self.vis:
            return logit_masks,vsim_maps,dsim_maps,tsim_maps,multi_sim
        else:
            return logit_masks
...
class MCS345l_TC345l(nn.Module):
    def __init__(self, backbone, use_original_imgsize, dropout=0.0, vis = False):
        super().__init__()
        # add supervision
        self.backbone_type = backbone
        ...
        self.TriFusel4 = TriCorr_v2_6(in_ch=self.multilevel_ch[-1],mid_ch=self.tri_mid_ch,out_ch=self.tri_out_ch,dropout=dropout)
        self.TriFusel3 = TriCorr_v2_6(in_ch=self.multilevel_ch[-2],mid_ch=self.tri_mid_ch,out_ch=self.tri_out_ch,dropout=dropout)
        self.TriFusel2 = TriCorr_v2_6(in_ch=self.multilevel_ch[-3],mid_ch=self.tri_mid_ch,out_ch=self.tri_out_ch,dropout=dropout)

        self.Fusion = ConvModule(self.tri_out_ch+self.nbottlenecks[-1],self.tri_mid_ch,1)
        self.deblock1 = ConvModule(self.tri_out_ch+self.tri_mid_ch+self.nbottlenecks[-2],self.tri_out_ch,1)
        self.deblock2 = ConvModule(self.tri_out_ch*2+self.nbottlenecks[-3],self.tri_out_ch,1)
    ...
    def forward(self, query_img, query_img_th, query_img_d,  support_img, support_img_th, support_img_d, support_mask):
        ...
        Trimap_l2 = self.TriFusel2([query_feats[-3][-1],query_feats_d[-3][-1],query_feats_th[-3][-1]],
                                 [support_feats[-3][-1],support_feats_d[-3][-1],support_feats_th[-3][-1]])
        Trimap_l2 = torch.cat(Trimap_l2,dim=1)
        ...
        out1 = self.deblock2(torch.cat([Trimap_l2,self._upsample_like(out2,multi_sim[-3]),multi_sim[-3]],dim=1))
        logit_masks.append(self.side1(out1))
...
class MCSwo_TC345l(nn.Module):
    def __init__(self, backbone, use_original_imgsize, dropout=0.0, vis = False):
        ...
        self.Fusion = ConvModule(self.tri_out_ch,self.tri_mid_ch,1)
        self.deblock1 = ConvModule(self.tri_out_ch+self.tri_mid_ch,self.tri_out_ch,1)
        self.deblock2 = ConvModule(self.tri_out_ch*2,self.tri_out_ch,1)
    ...
    def forward(self, query_img, query_img_th, query_img_d,  support_img, support_img_th, support_img_d, support_mask):
        ...
        out3 = self.Fusion(torch.cat([Trimap_l4],dim=1))
        logit_masks.append(self.side3(out3))

        out2 = self.deblock1(torch.cat([Trimap_l3,self._upsample_like(out3,Trimap_l3)],dim=1))
        logit_masks.append(self.side2(out2))

        out1 = self.deblock2(torch.cat([Trimap_l2,self._upsample_like(out2,Trimap_l2)],dim=1))
        logit_masks.append(self.side1(out1))
        ...
        # decoder output 2 channel: indicate probabilities of foreground and background
        if self.vis:
            return logit_masks,vis_l3,vis_l4
        else:
            return logit_masks
...
class MCSpro345l_TC345l(nn.Module):
    def __init__(self, backbone, use_original_imgsize, dropout=0.0, vis = False):
        super().__init__()
        # add supervision
        self.backbone_type = backbone
        ...
        # self.corr_mid = corr_mid*3 if len(corr_mid)==1 else corr_mid # correspond to l2,l3,l4
        ################################################## change is here ##################################################
        self.multi_smi_fuse = nn.ModuleList([ConvModule(self.nbottlenecks[i]*3,self.nbottlenecks[i]*3,1) 
                                             for i in range(len(self.nbottlenecks))]) # correspond to l2,l3,l4
        ...
        self.Fusion = ConvModule(self.tri_out_ch+self.nbottlenecks[-1]*3,self.tri_mid_ch,1)
        self.deblock1 = ConvModule(self.tri_out_ch+self.tri_mid_ch+self.nbottlenecks[-2]*3,self.tri_out_ch,1)
        self.deblock2 = ConvModule(self.tri_out_ch*2+self.nbottlenecks[-3]*3,self.tri_out_ch,1)
...
class MCSpro345l_TC45l(nn.Module):
    # namely HS_VDT_v2_10
    def __init__(self, backbone, use_original_imgsize, dropout=0.0, vis = False):
        super().__init__()
        # add supervision
        ...
        self.Fusion = ConvModule(self.tri_out_ch+self.nbottlenecks[-1]*3,self.tri_mid_ch,1)
        self.deblock1 = ConvModule(self.tri_out_ch+self.tri_mid_ch+self.nbottlenecks[-2]*3,self.tri_out_ch,1)
        self.deblock2 = ConvModule(self.tri_out_ch+self.nbottlenecks[-3]*3,self.tri_out_ch,1)
...
class MCSwo_TC45l(nn.Module):
    ...
    def forward(self, query_img, query_img_th, query_img_d,  support_img, support_img_th, support_img_d, support_mask):
        ...
        out1 = self.deblock2(self._upsample_like(out2,query_feats[-3][-1]))
        logit_masks.append(self.side1(out1))
...
class MCS345l_TCwo(nn.Module):
    ...
    def __init__(self, backbone, use_original_imgsize, dropout=0.0, vis = False):
        ...
        self.Fusion = ConvModule(self.nbottlenecks[-1],self.tri_mid_ch,1)
        self.deblock1 = ConvModule(self.tri_mid_ch+self.nbottlenecks[-2],self.tri_out_ch,1)
        self.deblock2 = ConvModule(self.tri_out_ch+self.nbottlenecks[-3],self.tri_out_ch,1)
    ...
    def forward(self, query_img, query_img_th, query_img_d,  support_img, support_img_th, support_img_d, support_mask):
        ...
        out3 = self.Fusion(multi_sim[-1])
        logit_masks.append(self.side3(out3))

        out2 = self.deblock1(torch.cat([self._upsample_like(out3,multi_sim[-2]),multi_sim[-2]],dim=1))
        logit_masks.append(self.side2(out2))
...
class MCS345l_TC45l_D(nn.Module):
    # namely HS_VDT_v2_10
    def __init__(self, backbone, use_original_imgsize, dropout=0.0, vis = False):
        super().__init__()
        # add supervision
    ...
    def forward(self, query_img, query_img_th, query_img_d,  support_img, support_img_th, support_img_d, support_mask):
        ...
        ############## D is assigned as main modality #############
...
class MCS345l_TC45l_T(nn.Module):
    # namely HS_VDT_v2_10
    def __init__(self, backbone, use_original_imgsize, dropout=0.0, vis = False):
        super().__init__()
        # add supervision
    ...
    def forward(self, query_img, query_img_th, query_img_d,  support_img, support_img_th, support_img_d, support_mask):
        ...
        ############## T is assigned as main modality #############
        Trimap_l4 = self.TriFusel4([query_feats_th[-1][-1],query_feats[-1][-1],query_feats_d[-1][-1]],
                                 [support_feats_th[-1][-1],support_feats[-1][-1],support_feats_d[-1][-1]]) # features = [fV,fD,fT]
        Trimap_l4 = torch.cat(Trimap_l4,dim=1)
        if(self.vis):vis_l4 = [query_feats[-1][-1],query_feats_d[-1][-1],query_feats_th[-1][-1],Trimap_l4]


        Trimap_l3 = self.TriFusel3([query_feats_th[-2][-1],query_feats[-2][-1],query_feats_d[-2][-1]],
                                 [support_feats_th[-2][-1],support_feats[-2][-1],support_feats_d[-2][-1]])
        Trimap_l3 = torch.cat(Trimap_l3,dim=1)
        if(self.vis):vis_l3 = [query_feats[-2][-1],query_feats_d[-2][-1],query_feats_th[-2][-1],Trimap_l3]
