...
class TriCorr_v2_6(nn.Module):
    ...
    def forward(self, query, support):
        ...
        RE = self.Q_sim_conv(sim)*QV
        Q_en_V = self.MHA_V(self.conv_DT(torch.cat([QD,QT],dim=1)),RE,RE)
        
        RE = self.S_sim_conv(sim)*SV
        S_en_V = self.MHA_V(self.conv_DT(torch.cat([SD,ST],dim=1)),RE,RE)
