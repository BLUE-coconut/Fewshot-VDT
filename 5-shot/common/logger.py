...
class AverageMeter:
    ...
    def compute_iou(self):
        ...
        miou = iou[1].mean() * 100
        fb_in = self.intersection_buf.index_select(1, self.class_ids_interest).sum(dim=1)
        fb_un = self.union_buf.index_select(1, self.class_ids_interest).sum(dim=1)
        fb_iou = (fb_in / fb_un).mean() * 100
...
class Logger:
    ...
    def initialize(cls, args, training, modelname = None):
        ...
        cls.logpath = os.path.join('logs', logpath +"_"+ modelname + "_fold" + str(args.fold) + "_" + str(args.shot) + "shot" + '.log')
    ...
    def save_ckp(cls, optimizer,epoch):
        ...
        torch.save(checkpoint, os.path.join(cls.logpath, 'best_ckp.pt'))
